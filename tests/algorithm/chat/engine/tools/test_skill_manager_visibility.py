...
def test_skill_list_filters_prompt_list_read_and_run(tmp_path, monkeypatch):
    ...
    manager = SkillManager(
        dir=str(tmp_path), skills=['visible-skill'],
        sandbox=SimpleNamespace(execute_script=execute_script),
    )
    ...
    assert shell_calls
    assert os.path.basename(shell_calls[0]['source_dir']) == 'visible-skill'
...
def test_read_reference_rejects_unsafe_rel_path(tmp_path, rel_path):
    ...
    with pytest.raises(ToolExecutionError, match='Invalid reference path.*visible-skill'):
