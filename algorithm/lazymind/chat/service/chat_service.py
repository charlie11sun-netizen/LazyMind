...
async def _handle_chat_impl(
    ...
    *,
    ...
    active_workflow_tool_isolation = bool(
        isinstance(effective_plugin_context, dict)
        and effective_plugin_context.get('session_id')
        and plugin_tools
    ...
    if active_workflow_tool_isolation:
        ...
        # The plugin's declarative rerun_when metadata still decides the owning step.
        active_configs = []
        attachment_configs = []
        all_tools = [intentwriter, *ask_user_tools, *plugin_tools]
        LOG.info(
            '[ChatServer] [ACTIVE_WORKFLOW_TOOL_ISOLATION] [sid=%s] '
            '[plugin_id=%s] [outcome=%s] [tools=%s]',
            conversation.session_id,
            effective_plugin_context.get('plugin_id'),
            task_profile.primary_outcome,
