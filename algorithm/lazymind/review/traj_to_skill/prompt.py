...
def resolution_prompt(candidate: dict[str, Any], global_skill_summaries: str) -> str:
    return (
        ...
        'Classify by the reusable capability boundary, using only the applicable scenario and '
        'task goal to decide whether a new skill is justified. Procedure differences improve an '
        'existing skill; they do not define a new skill.\n\n'
        'Existing source skills are the first comparison set:\n'
        '- CANDIDATE_SKILL.source_skills records existing skills used by the source trajectories;\n'
        '- first decide whether the candidate is reusable guidance for one of those source skills;\n'
        '- when multiple source skills exist, choose the closest workflow match among them before '
        'considering other existing skills;\n'
        '- a new skill is justified only when both the applicable scenario and task goal differ '
        'from every existing skill.\n\n'
        'Decision rules:\n'
        '- choose type="patch" when either the applicable scenario or task goal is the same;\n'
        '- a broader, narrower, or adjacent scenario with the same task goal extends the existing skill;\n'
        '- added, removed, reordered, or refined steps are procedure enhancements and must be patched;\n'
        '- confirmation gates, user choices, branches, recovery paths, tool changes, data sources, '
        'parameters, research depth, and output format are procedure enhancements and must not '
        'justify type="new";\n'
        '- choose type="new" only when applicable_scenario_same=false AND task_goal_same=false;\n'
        '- a new candidate name or more specialized description is never sufficient evidence for new.\n\n'
        'Target selection rules:\n'
        '- patch_skill_key MUST exactly match the full key shown in bold for one provided existing skill '
        '(for example, "internal/example");\n'
        '- do not invent skill keys;\n'
        '- when comparing against an existing or source skill, return its key as patch_skill_key even '
        'if your initial classification is type="new"; use an empty key only when no related existing '
        'workflow exists.\n\n'
        'Return ONLY valid JSON with these keys:\n'
        '- type: "new" or "patch"\n'
        '- patch_skill_key: the best matching existing skill key, or an empty string only when none exists\n'
        '- applicable_scenario_same: whether the candidate and selected existing skill apply to the '
        'same, overlapping, broader, narrower, or adjacent user situation\n'
        '- task_goal_same: whether they aim to achieve the same user outcome or deliverable purpose\n'
        '- procedure_enhancements: concise descriptions of reusable procedural differences to merge; '
