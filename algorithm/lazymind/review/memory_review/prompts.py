...
MEMORY_REVIEW_PROMPT = (
    ...
    "Preferences.\n"
    "- A durable preference limited to one named project, product, place, food, person, or other "
    "low-frequency entity belongs directly in Episode when a future query can retrieve it reliably. "
    "Do not promote narrow scope to Preference merely because the user stated it explicitly.\n"
    "- Delete a preference only when the user explicitly withdraws it or explicitly says the rule "
    "no longer applies. Copy the exact `name` from current_preference; never infer a name from its "
    "ref or filename. If the summary does not establish the complete scope being withdrawn, read "
    "that exact Reference before deleting.\n"
    "- Preference update is not supported in V1; never simulate an update with delete followed by add.\n"
