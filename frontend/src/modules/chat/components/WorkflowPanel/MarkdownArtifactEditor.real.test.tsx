...
it.each([false,true])('preserves unsaved source edits when returning to rich text (reopening more menu: %s)',async(viaPreview)=>{
 ...
 expect(onSave.mock.calls[0][0]).toBe(source.replace('Source','Rich'));
});
...
it('keeps a newer source baseline when an older rich save finishes',async()=>{
 ...
 const {container,getByRole}=render(<MarkdownArtifactEditor markdown={'Old https://example.org\n'} sourceRevision={1} onSave={onSave} />);
 await waitFor(()=>expect(container.querySelector('[contenteditable=true]')).toHaveTextContent('Old'));
 ...
 await waitFor(()=>expect(onSave).toHaveBeenCalledTimes(2),{timeout:2500});
 expect(onSave.mock.calls[1][0]).toBe(source.replace('Source','Second'));
});
...
it.each([
 ...
 {original:'Old plain\n',source:'Old plain\n\n',viaPreview:true},
 {original:'First\n\nSecond\n',source:'First\n\n\nSecond\n',viaPreview:false},
])('saves explicit source whitespace after switching back to rich: %j',async({original,source,viaPreview})=>{
 const onSave=vi.fn(async(markdown:string)=>({markdown,revision:2}));
 const {container,getByRole}=render(<MarkdownArtifactEditor markdown={original} sourceRevision={1} onSave={onSave} />);
...
it.each([false,true])('does not save a reverted source edit or an unchanged mode switch: %s',async(revert)=>{
...
it('keeps rich normalization tracked after saving only source margins',async()=>{
 const original='Old https://example.org A & B.\n';
 ...
 await act(async()=>capture.change?.(capture.editor!.getMarkdown(),false));
 await waitFor(()=>expect(onSave).toHaveBeenCalledTimes(2),{timeout:2500});
 expect(onSave.mock.calls[1][0]).toBe(source.replace('Old','New'));
});
