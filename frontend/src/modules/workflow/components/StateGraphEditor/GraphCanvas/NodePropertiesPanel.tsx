...
interface Props {
  node: StepNode;
  model: GraphModel;
  /** Workflow metadata model — provides tool function lists. */
...
export default function NodePropertiesPanel({ node, model, workflowModel, scenarioData, onScenarioChange, onClose, onChange, onDelete, disableAddTransition, readonly = false, visualContent }: Props) {
  ...
  // Grouped tool options: system tools first, then workflow script functions.
  const workflowFunctions: string[] = workflowModel?.tool_scripts
    ? workflowModel.tool_scripts.flatMap((ts) => ts.functions)
    : [];
  ...
  if (workflowFunctions.length > 0) {
    toolFunctionOptions.push({
       label: t('selfEvolutionRun.nodePropsWorkflowTools'),
      options: workflowFunctions.map((fn) => ({ label: fn, value: fn })),
    });
  }
