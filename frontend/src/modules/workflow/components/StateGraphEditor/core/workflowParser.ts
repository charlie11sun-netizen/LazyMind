...
function parseSlots(raw: unknown): WorkflowSlotDef[] {
  if (!raw) return [];
  // New format: array of objects with an 'id' field.
  if (Array.isArray(raw)) {
    return raw.flatMap((item): WorkflowSlotDef[] => {
      if (!item || typeof item !== 'object' || Array.isArray(item)) return [];
      const entry = item as Record<string, unknown>;
      const id = String(entry.id ?? '').trim();
      if (!id) return [];
      const slot: WorkflowSlotDef = {
        id,
        type: ['text', 'image', 'file', 'json'].includes(String(entry.type)) ? (String(entry.type) as WorkflowSlotDef['type']) : 'text',
  ...
  if (typeof raw === 'object' && !Array.isArray(raw)) {
    return Object.entries(raw as Record<string, unknown>).flatMap(([id, val]): WorkflowSlotDef[] => {
...
function parseToolScripts(raw: unknown): WorkflowToolScript[] {
  if (!Array.isArray(raw)) return [];
  return raw.flatMap((item): WorkflowToolScript[] => {
    if (!item || typeof item !== 'object' || Array.isArray(item)) return [];
    const entry = item as Record<string, unknown>;
...
function parseUiTabs(raw: unknown): { tabs: WorkflowUiTab[]; slots?: Record<string, WidgetConfig> } | undefined {
  ...
  const tabs = uiObj.tabs.flatMap((tab): WorkflowUiTab[] => {
    ...
    const layout = validLayouts.includes(rawLayout) ? (rawLayout as WorkflowUiTab['layout']) : undefined;
...
/**
 * Parse a workflow.yaml string into a WorkflowModel.
 * Returns null on YAML syntax errors.
 */
export function parseWorkflowYaml(yamlText: string): WorkflowModel | null {
  let raw: RawWorkflowYaml;
  try {
    raw = (jsYaml.load(yamlText) ?? {}) as RawWorkflowYaml;
  } catch {
    return null;
  }
