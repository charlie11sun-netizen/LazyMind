...
/**
 * Serialize a WorkflowModel back to a canonical workflow.yaml YAML string.
 * Slots are sourced from GraphModel.slots (single source of truth) rather than
 * WorkflowModel.slots (which is no longer maintained).
 * i18n block is preserved as-is.
 */
export function serializeWorkflowModel(model: WorkflowModel, graphModel?: GraphModel): string {
  ...
  // workflow.yaml declarations correct for canvas renames and direct state.yml
  // edits, even if WorkflowModel was initialized from an incomplete draft.
  ...
  // Slots come from GraphModel.slots when available; fall back to WorkflowModel.slots for
