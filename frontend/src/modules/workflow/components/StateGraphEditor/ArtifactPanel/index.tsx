...
import type { WorkflowModel, WorkflowUiTab, WidgetConfig, WidgetType, CompositePanelNode } from '../core/workflowModel';
import { SLOT_DEFAULT_WIDGET, SLOT_COMPATIBLE_WIDGETS } from '../core/workflowModel';
...
interface Props {
  ...
  activeTabId?: string;
  onUiModelChange?: (ui: WorkflowModel['ui']) => void;
...
/** Describes where a slot lives within the workflow UI. */
type SlotLocation =
...
/** Find where a slot lives across all WorkflowUiTabs. */
function findSlotLocation(slotId: string, tabs: WorkflowUiTab[]): SlotLocation | null {
...
/** Collect all assignable positions from a WorkflowUiTab list as flat entries. */
function collectAssignTargets(tabs: WorkflowUiTab[], slotMap: Record<string, SlotDef>, blockLabelFallback: (path: number[]) => string): AssignTarget[] {
...
// ── Main component ───────────────────────────────────────────────────────────
export default function ArtifactPanel({ model, onClose, onModelChange, uiMode, inline, workflowModel, onUiModelChange, onTabNavigate, readonly = false, startAddingToken = 0 }: Props) {
  const { t } = useTranslation();
  ...
  const assignSlot = (target: AssignTarget, slotId: string, widget: WidgetConfig) => {
    if (!workflowModel || !onUiModelChange) return;
    const nextUiSlots = { ...(workflowModel.ui?.slots ?? {}), [slotId]: widget };
    ...
    if (!target.isComposite) {
      ...
      onUiModelChange({ ...(workflowModel.ui ?? { tabs: [] }), tabs: newTabs, slots: nextUiSlots });
    } else {
  ...
  const removeSlotFromUi = (slotId: string) => {
    ...
    onUiModelChange({ ...(workflowModel.ui ?? { tabs: [] }), tabs: newTabs });
  };

  const updateWidget = (slotId: string, widget: WidgetConfig) => {
    if (!workflowModel || !onUiModelChange) return;
    const nextUiSlots = { ...(workflowModel.ui?.slots ?? {}), [slotId]: widget };
    onUiModelChange({ ...(workflowModel.ui ?? { tabs: [] }), tabs, slots: nextUiSlots });
  };
