...
import type { WorkflowModel, WorkflowUiTab, WorkflowSlotDef, WidgetConfig, WidgetType, CompositePanelNode } from '../core/workflowModel';
...
function resolveSlot(
  ...
  workflowSlotMap: Record<string, WorkflowSlotDef>,
) {
  const g = slotMap[slotId];
  const p = workflowSlotMap[slotId];
  return {
...
function getWidgetConfig(
  ...
  const { type, cardinality } = resolveSlot(slotId, slotMap, workflowSlotMap);
...
function renderCompositeNode(
  ...
  if (node.slot) {
    const widget = getWidgetConfig(node.slot, uiSlots, slotMap, workflowSlotMap);
    const label = resolveSlot(node.slot, slotMap, workflowSlotMap).label;
    return <WidgetPlaceholder widgetConfig={widget} label={label} />;
  }

  if (node.tabs && node.tabs.length > 0) {
    const firstTab = node.tabs[0];
    const widget = firstTab.slot ? getWidgetConfig(firstTab.slot, uiSlots, slotMap, workflowSlotMap) : { widgetType: 'text-single' as const };
    const label = firstTab.slot ? resolveSlot(firstTab.slot, slotMap, workflowSlotMap).label : firstTab.label;
    return (
      ...
      <div className={`wywp-composite-container wywp-composite-container--${node.direction}`}>
        {node.children.map((child, idx) => {
          ...
          return (
            ...
            >
              {renderCompositeNode(child, uiSlots, slotMap, workflowSlotMap)}
            </div>
          );
        })}
      </div>
...
interface TabContentProps {
  ...
  onCompositeTabPositionChange?: (pos: WorkflowUiTab['composite_tab_position']) => void;
}
    ...
    if (tab.composite_layout?.direction) {
      return (
        <div className='wywp-layout-composite'>
          {renderCompositeNode(tab.composite_layout, uiSlots, slotMap, workflowSlotMap)}
        </div>
      );
    }
    return (
      <div className='wywp-layout-vertical'>
        {tab.slots.map((s) => {
          const widget = getWidgetConfig(s.id, uiSlots, slotMap, workflowSlotMap);
          const label = resolveSlot(s.id, slotMap, workflowSlotMap).label;
...
interface Props {
  ...
  activeTabId?: string;
  activeLayout?: WorkflowUiTab['layout'];
  ...
  onLayoutChange?: (layout: WorkflowUiTab['layout']) => void;
...
export default function UiWysiwygPreview({
  ...
  const { t } = useTranslation();
  const tabs = workflowModel.ui?.tabs ?? [];
  const uiSlots: Record<string, WidgetConfig> = (workflowModel.ui?.slots ?? {}) as Record<string, WidgetConfig>;
  const workflowSlotMap = Object.fromEntries(workflowModel.slots.map((s) => [s.id, s]));
  ...
  const startEdit = (tab: WorkflowUiTab) => {
    setEditingId(tab.id);
      ...
      <div className="wywp-content">
        {activeTab && (
          <TabContent
            ...
            uiSlots={uiSlots}
            slotMap={slotMap}
            workflowSlotMap={workflowSlotMap}
