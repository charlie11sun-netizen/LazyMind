...
![macOS](https://img.shields.io/badge/macOS-arm64-111827?style=flat-square\&logo=apple)
![Windows](https://img.shields.io/badge/Windows-x64-0078D4?style=flat-square\&logo=windows)
...
LazyMind 围绕知识密集型工作设计了七类核心场景，每类场景都有明确的执行路径和交付标准。你不需要全程盯着 Agent，而是选择知识与工作流后，让 LazyMind 持续规划、执行、展示中间结果，并在关键节点等待你的确认与反馈。

所有场景共享同一套知识底座、Skill 系统和评测闭环，确保执行过程可追溯、产物可编辑、能力可持续改进。
...
[https://github.com/user-attachments/assets/ebc2440e-86f8-4117-a917-62ce4e79a117](https://github.com/user-attachments/assets/ebc2440e-86f8-4117-a917-62ce4e79a117)

> 上方视频展示 LazyMind 从任务输入到可编辑产物的完整执行过程。

### 调研与评审

调研场景覆盖从资料收集到最终报告产出的完整链路。LazyMind 首先根据你的任务描述制定调研计划，确定搜索范围与优先级；随后在本地知识库、网络和学术数据库中并行检索，提取关键证据并进行交叉验证；接下来对不同来源的观点进行对比分析，识别共识与分歧；最后综合各方信息形成结构化报告，并标注每一条论断的来源。

你可以随时在关键节点审批或修改中间产物，系统会记录完整版本历史，支持回溯到任意阶段重新调整，而不必从头开始。

### AI Writer

AI Writer 接收素材（文档、链接或自然语言描述）后，首先对素材进行整理与结构化，识别核心论点与支撑材料；然后根据任务目标生成大纲，并与你确认章节安排；接着按章节逐步写作，每章完成后展示 Artifact 并等待确认；最后进行全文修改、术语统一与格式处理，经终审后输出可编辑文档。

整个过程中，素材引用、大纲版本和修改记录全部保留，你可以在任意节点介入编辑，AI 则在后续章节中继续遵循你的调整方向。

### 演示文稿

演示文稿场景从需求确认开始，明确演示目标、受众和时长要求后，收集相关资料并生成幻灯片大纲；你确认大纲后，系统开始逐页生成幻灯片，每页同时附带演讲稿；生成过程中你可以用自然语言修改任意页面的内容或样式；完成后支持导出为 PDF、图片版 PPTX 或包含所有元素的完整可编辑 PPTX。

素材来源支持用户输入、上传文件、LazyMind 知识库、Web 搜索结果和 AI 配图，确保内容既有内部资料支撑，也有外部信息补充。

### AI Image

AI Image 场景首先理解你的图像需求，包括主题、风格、参考示例和用途说明；随后在本地知识库或网络中收集参考图像，分析构图、色彩和风格特征；接着优化生成 Prompt，结合参考信息与原始需求；最后生成或编辑图像，并保留完整的生成过程记录，包括中间版本和 Prompt 调整历史。

对于动态表情或系列图像，系统会保存所有版本并支持对比选择，确保创作过程可追溯。

### 知识助手

知识助手场景将本地文件、飞书、Notion 和对象存储等数据源统一接入知识库；文档经过解析（PDFReader、MinerU 或 PaddleOCR-VL）后，通过多路 Embedding、混合检索和重排返回结果；你可以在检索结果中直接查看原文引用，也可以就当前文档进行对话追问。

所有回答都标注来源信息，临时会话不会污染普通聊天历史，也可以在需要时转为正式会话继续保存，确保知识沉淀可追溯。

### 外部 Agent 增强

外部 Agent 增强让 Codex、Cursor、WorkBuddy、TRAE Work、DeepSeek Harness 等工具能够调用 LazyMind 的知识、Skill 和 Workflow 能力。LazyMind 发现这些 Agent 的本地项目和历史任务后，你可以在统一工作台中创建、继续和查看执行过程。

外部 Agent 通过 MCP 协议访问授权范围内的知识库和工作流，而无需重复配置任务材料。你可以在 LazyMind 中查看外部 Agent 的调用记录和执行结果，保持跨平台任务的一致性。

### 质量改进

质量改进场景将日常使用中积累的 badcase 转化为评测样本，通过基线评测确定改进前的系统表现；随后诊断问题根因，区分是知识缺失、Prompt 偏差还是模型能力不足；接着制定修复方案并实施改进；最后通过 A/B Test 验证改进效果，安全部署到生产环境。

偏好与术语由"智积阅累"持续沉淀，经验与 Skill 由团队共享，而所有改动都经过评测验证，而不是直接推送未检查的 Prompt 改动。
