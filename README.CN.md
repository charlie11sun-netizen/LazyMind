...
LazyMind 在团队部署模式下提供完整的企业级安全合规与权限控制能力，涵盖多租户数据隔离、RBAC 细粒度权限管控与操作审计日志三大核心维度。

**多租户数据隔离**：通过租户级别的数据分区，确保不同租户的知识库、工作流、Skill 和 Artifact 完全隔离。数据层采用租户 ID 标记结合数据库行级安全策略，应用层通过中间件统一注入租户上下文，防止跨租户数据访问。

**RBAC 细粒度权限管控**：LazyMind 引入基于角色的访问控制模型，支持为用户分配多角色并定义细粒度权限。权限粒度覆盖知识库读写、工作流执行、Skill 安装与版本管理、Artifact 访问与编辑等操作，并支持按租户、团队和项目维度配置，实现最小权限原则。

**操作审计日志**：LazyMind 记录所有关键操作的审计日志，包括用户登录、知识库访问、工作流执行、权限变更等行为，支持日志查询、导出和合规审计。

![企业级多租户隔离与安全权限架构示意图](assets/a6/a6c806860fd90d2ad7e3d6ac9e260ecd3301a93033a58cb6de8e0a41f62cd8ae.jpg)
