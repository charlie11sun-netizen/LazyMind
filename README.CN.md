...
LazyMind 在团队部署场景下提供完整的企业级安全合规能力，确保多租户数据隔离、细粒度权限管控与操作可追溯审计。

**多租户数据隔离**：LazyMind 通过 Core ACL 实现租户级别的数据隔离，每个租户的知识库、工作流、Skill 与产物相互独立，跨租户无法访问对方数据。Desktop Mode 与团队部署使用同一套数据模型，本地数据遵循平台规范存储在用户目录下，团队部署时自动切换为集中式安全存储。

**RBAC 细粒度权限管控**：LazyMind 集成 Kong API 网关与 JWT/RBAC 体系，支持按角色配置知识库访问、工作流执行、Skill 安装与系统设置等操作的权限。管理员可以为不同团队或项目配置差异化的权限策略，普通用户仅能使用已授权的知识与能力。

**操作审计日志**：所有关键操作（包括知识库访问、Workflow 执行、权限变更与系统配置）均生成完整审计日志，记录操作人、时间、操作类型与影响范围，便于合规审查与问题溯源。

![企业级多租户隔离与安全权限架构示意图](assets/74/74651e72dabe2ec56d08e717298dd929cda16250032c9531a8b923b82fd89f0f.jpg)
