...
企业级部署场景下，LazyMind 从多租户数据隔离、RBAC 细粒度权限管控与操作审计日志三个层面提供完整的安全合规保障。

**多租户数据隔离**：LazyMind 通过租户级别的数据分区实现严格的数据隔离，每个租户拥有独立的数据存储空间与索引，确保不同租户间的知识库、Skill、Workflow 和执行记录互不可见，防止跨租户数据泄露。

**RBAC 细粒度权限管控**：基于角色的访问控制支持到功能模块、数据范围与操作类型的精细授权。管理员可以为不同角色配置知识库访问权限、Workflow 执行权限、Skill 管理权限和系统配置权限，实现最小权限原则。

**操作审计日志**：系统完整记录所有关键操作的审计日志，包括用户登录、知识库访问、Workflow 执行、权限变更和系统配置等，所有日志支持查询与导出，满足企业安全审计与合规要求。

![企业级多租户隔离与安全权限架构示意图](assets/ac/aca785bca740f1e339c71727f6686a99e204d9b7b588de882dceed7cfb845c48.jpg)
