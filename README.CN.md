...
LazyMind 在团队部署场景下提供完善的企业级安全合规与权限管控能力。通过多租户数据隔离机制，每个租户的数据、配置和产物在存储层和访问层严格分离，确保不同组织之间的信息不会产生交叉泄露。RBAC（基于角色的访问控制）支持细粒度权限管控，可按用户、角色、部门和项目等维度灵活配置访问策略，覆盖知识库、Workflow、Skill、任务和 Artifact 等核心资源的读写与审批权限。系统同时保留完整的操作审计日志，记录所有关键操作的执行者、时间、操作类型与影响范围，并支持导出与合规审查。

![企业级多租户隔离与安全权限架构示意图](assets/95/953f97a3d3a98bec4670a01a9dcbd2d8cd5eafd2b6e4d124949d462e97f3b7df.jpg)
