...
企业部署支持多租户数据隔离，通过租户 ID 标记与 Core ACL 实现租户间的资源与数据完全隔离，确保各租户的敏感信息不会跨边界泄露。RBAC 细粒度权限管控覆盖用户、角色、权限三个维度，支持按组织、任务与文档粒度分配访问与操作权限，并可结合 JWT/Kong 实现统一的身份认证与权限校验。所有关键操作——包括登录登出、数据访问、任务创建与配置变更——均生成不可篡改的操作审计日志，支持按时间、用户与操作类型查询与导出，满足企业级安全合规要求。

![企业级多租户隔离与安全权限架构示意图](assets/ca/ca2b7845890d4894c28c485e0711c1f0cedb05a7e1e958ca3d1fc09af85fc53a.jpg)
