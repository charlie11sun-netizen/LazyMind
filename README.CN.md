...
LazyMind 提供企业级安全合规与权限管控能力，覆盖多租户数据隔离、RBAC 细粒度权限管控与操作审计日志三大核心模块，确保团队协作环境下的数据安全与合规可追溯。

**多租户数据隔离**：基于租户维度的数据分区，租户间的知识库、任务、Artifact 与执行记录完全隔离；团队部署模式下，Kong 网关与 Core ACL 协同实现请求级别的租户身份验证与资源访问控制，防止跨租户数据泄露。

**RBAC 细粒度权限管控**：基于角色的访问控制（RBAC）支持对用户、团队与外部 Agent 分配细粒度权限，覆盖知识库读写、工作流执行、Artifact 审批与系统配置等操作；权限层级与 Skill/Workflow 绑定，支持按需最小授权与权限继承。

**操作审计日志**：完整记录知识库访问、Workflow 执行、Artifact 修改与权限变更等关键操作，审计日志支持按时间、用户与操作类型筛选，并可关联至具体的执行步骤与产物版本，满足企业合规审查与安全追溯需求。

![企业级多租户隔离与安全权限架构示意图](assets/59/59fdb2c1ea8272838c760817ebe0cf58b518316d2c1b78ec924956c29e4abb8f.jpg)
