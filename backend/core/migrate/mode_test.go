...
func TestRepositoryStructuredMigrationCatalogLoads(t *testing.T) {
	...
	if strings.Contains(string(v03Up), "ADD CONSTRAINT chk_resource_update_tasks_task_type") {
		t.Fatal("v0_3 aggregate up must not restore the application-defined resource update task type check")
	}
	v03Down, err := os.ReadFile(v03.Aggregate.DownPath)
	if err != nil {
		t.Fatalf("read v0_3 aggregate down: %v", err)
	}
	for _, token := range []string{"enable_plugin_was_null", "DROP TABLE IF EXISTS conversation_policy_snapshot_backups", "DROP COLUMN IF EXISTS pinned_at", "DROP COLUMN IF EXISTS free_auto_select_priority", "DROP COLUMN IF EXISTS free_auto_select_base_urls", "DROP COLUMN IF EXISTS source_context", "DROP COLUMN IF EXISTS source_selected_text", "DROP COLUMN IF EXISTS source_seq", "DROP COLUMN IF EXISTS source_history_id", "DROP COLUMN IF EXISTS relation_type", "DROP COLUMN IF EXISTS parent_conversation_id", "DROP COLUMN IF EXISTS chat_model_version", "DROP COLUMN IF EXISTS chat_model_snapshot", "DROP COLUMN IF EXISTS chat_model_id", "DROP COLUMN IF EXISTS chat_model_mode", "DROP COLUMN IF EXISTS writing_subtasks", "idx_conversations_parent_relation", "ON public.skills(owner_user_id, category, skill_name)", "AS identity_conflicts", "AS relative_root_conflicts"} {
