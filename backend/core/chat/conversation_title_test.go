...
func conversationTitleTestService(t *testing.T) *conversationTitleService {
	t.Helper()
	...
	s := newConversationTitleService(db)
...
func conversationTitleTestResult(status string) algo.ConversationTitleResult {
	return algo.ConversationTitleResult{Status: "succeeded", Output: algo.ConversationTitle{Title: "对话整理方案", Summary: "为 LazyMind 设计对话整理方案。", IntentStatus: status, MissingContext: []string{}}, Usage: json.RawMessage(`{"model_calls":1,"model_id":{"model":"test"}}`)}
}
func conversationTitleTestConversation(t *testing.T, s *conversationTitleService, id, title, source string) orm.Conversation {
	t.Helper()
...
func conversationTitleTestInput(t *testing.T, s *conversationTitleService, id, conv, text string, seq int) {
	t.Helper()
...
func conversationTitleTestRun(t *testing.T, s *conversationTitleService, conv string) (asyncjob.Result, error) {
	t.Helper()
...
func conversationTitleTestMeta(t *testing.T, s *conversationTitleService, id string) orm.ConversationOpening {
	t.Helper()
...
func TestOpeningTitleProtectionAndStableActivity(t *testing.T) {
	for _, source := range []string{"unknown", "user", "rename_during_call"} {
		t.Run(source, func(t *testing.T) {
			...
			before := conversationTitleTestConversation(t, s, "c1", title, actualSource)
			conversationTitleTestInput(t, s, "h1", "c1", title, 1)
			...
			if _, err := conversationTitleTestRun(t, s, "c1"); err != nil {
				t.Fatal(err)
			}
			...
			m := conversationTitleTestMeta(t, s, "c1")
			if m.Summary == "" || !m.WindowClosed || m.CallCount != 1 {
				t.Fatalf("metadata: %+v", m)
			}
			conversationTitleTestInput(t, s, "h2", "c1", "顺便实现前端", 2)
...
func TestOpeningCompletionAndEvidenceReplacement(t *testing.T) {
	s := conversationTitleTestService(t)
	conversationTitleTestConversation(t, s, "c1", "优化检索", "default")
	conversationTitleTestInput(t, s, "h1", "c1", "优化检索", 1)
	calls := 0
	s.call = func(context.Context, json.RawMessage, map[string]any, int) (algo.ConversationTitleResult, error) {
		calls++
		return conversationTitleTestResult("provisional"), nil
	}
	for turn := 1; turn <= 3; turn++ {
		if turn > 1 {
			conversationTitleTestInput(t, s, fmt.Sprintf("h%d", turn), "c1", fmt.Sprintf("补充对象 %d", turn), turn)
		}
		if queued, err := s.enqueue(context.Background(), "c1", ""); err != nil || !queued {
	...
	if m := conversationTitleTestMeta(t, s, "c1"); !m.WindowClosed || m.GenerationCount != 3 || calls != 3 {
...
func TestOpeningEmptyTurnsDoNotConsumeWindow(t *testing.T) {
	s := conversationTitleTestService(t)
	conversationTitleTestConversation(t, s, "c1", "新对话", "default")
	for i, text := range []string{"在吗", "嗯", "好的", "排查数据库连接池泄漏"} {
		conversationTitleTestInput(t, s, fmt.Sprintf("h%d", i+1), "c1", text, i+1)
	}
	calls := 0
	s.call = func(_ context.Context, input json.RawMessage, _ map[string]any, _ int) (algo.ConversationTitleResult, error) {
		calls++
		if !strings.Contains(string(input), "连接池泄漏") {
			return algo.ConversationTitleResult{Status: "succeeded", Output: algo.ConversationTitle{IntentStatus: "empty"}, Usage: json.RawMessage(`{"model_calls":1}`)}, nil
		}
		return conversationTitleTestResult("ready"), nil
	}
	if queued, err := s.enqueue(context.Background(), "c1", ""); err != nil || !queued {
	...
	meta := conversationTitleTestMeta(t, s, "c1")
...
func TestOpeningExplicitRetrySameConfiguration(t *testing.T) {
	...
	for _, code := range conversationTitleExplicitRetryErrorCodes {
	...
	for _, code := range []string{"invalid_output", "model_failed", "transport_error", "request_timeout", "rate_limited", "service_unavailable"} {
		if !conversationTitleShouldRetry(code, "same", "same") {
	...
	for _, code := range []string{"token_limit", "model_configuration", "authentication_failed", "invalid_request", "not_found"} {
		...
		if !conversationTitleShouldRetry(code, "old", "new") {
...
func TestOpeningAttachmentArrivalAndLongInput(t *testing.T) {
	s := conversationTitleTestService(t)
	c := conversationTitleTestConversation(t, s, "c1", "看看这个", "default")
	conversationTitleTestInput(t, s, "h1", "c1", "看看这个", 1)
	...
	if m := conversationTitleTestMeta(t, s, "c1"); m.GenerationCount != 1 {
	...
	snap, err := loadConversationTitleSnapshot(s.db, c)
...
func TestOpeningFallbackAndRetryBudget(t *testing.T) {
	for _, code := range []string{"token_limit", "transport_error", "authentication_failed"} {
		t.Run(code, func(t *testing.T) {
			s := conversationTitleTestService(t)
			conversationTitleTestConversation(t, s, "c1", "整理对话", "default")
			conversationTitleTestInput(t, s, "h1", "c1", "整理对话", 1)
			calls := []string{}
			s.call = func(_ context.Context, _ json.RawMessage, cfg map[string]any, _ int) (algo.ConversationTitleResult, error) {
				calls = append(calls, cfg["llm"].(map[string]any)["model"].(string))
				if code == "transport_error" {
					return algo.ConversationTitleResult{}, context.DeadlineExceeded
				}
			...
			for i := 0; i < 3; i++ {
				result, err := conversationTitleTestRun(t, s, "c1")
				if err == nil {
...
func TestOpeningSavedImageDescriptionAndCapacityFallback(t *testing.T) {
	...
	snapshot, err := loadConversationTitleSnapshot(s.db, c)
...
func TestOpeningBackfillDedupPauseAndUserIsolation(t *testing.T) {
	...
	conversationTitleTestConversation(t, s, "c1", "整理对话", "unknown")
	conversationTitleTestInput(t, s, "h1", "c1", "整理对话", 1)
	for _, action := range []string{"start", "start", "pause", "resume"} {
		recorder := httptest.NewRecorder()
		ConversationTitleBackfill(recorder, newSettingsRequest("POST", "/conversations/metadata-backfill", `{"action":"`+action+`"}`, "u1", nil))
	...
	if m := conversationTitleTestMeta(t, s, "c1"); !m.WindowClosed || m.CallCount != 1 {
		t.Fatal("history reran")
	}
	rec := httptest.NewRecorder()
	conversationTitleTestConversation(t, s, "live", "优化检索", "default")
	conversationTitleTestInput(t, s, "live-h", "live", "优化检索", 1)
	if _, err := s.enqueue(context.Background(), "live", ""); err != nil {
		t.Fatal(err)
	}
	if _, err := conversationTitleTestRun(t, s, "live"); err != nil {
		t.Fatal(err)
	}
	ConversationTitleBackfill(rec, newSettingsRequest("GET", "/conversations/metadata-backfill", "", "u2", nil))
...
func TestOpeningEligibilityAndArchiveDuringCall(t *testing.T) {
	for _, field := range []string{"archived_at", "deleted_at", "is_ephemeral", "chat_executor"} {
		t.Run(field, func(t *testing.T) {
			s := conversationTitleTestService(t)
			conversationTitleTestConversation(t, s, "excluded", "整理对话", "default")
			conversationTitleTestInput(t, s, "h1", "excluded", "整理对话", 1)
	...
	s := conversationTitleTestService(t)
	conversationTitleTestConversation(t, s, "archive", "整理对话", "default")
	conversationTitleTestInput(t, s, "h1", "archive", "整理对话", 1)
	s.enqueue(context.Background(), "archive", "")
	s.call = func(context.Context, json.RawMessage, map[string]any, int) (algo.ConversationTitleResult, error) {
		s.db.Model(&orm.Conversation{}).Where("id = ?", "archive").UpdateColumn("archived_at", time.Now())
		return conversationTitleTestResult("ready"), nil
	}
	if _, err := conversationTitleTestRun(t, s, "archive"); err != nil {
		t.Fatal(err)
	}
	if meta := conversationTitleTestMeta(t, s, "archive"); meta.Status != "skipped" || meta.Summary != "" {
...
func TestOpeningBackfillReadsOriginalHistoryAfterCompression(t *testing.T) {
	s := conversationTitleTestService(t)
	conv := conversationTitleTestConversation(t, s, "compressed", "设计销售报表", "unknown")
	inputs := []string{"设计销售报表", "按月份汇总", "只给方案，暂不实施", "后来改为排查登录故障"}
	for i, input := range inputs {
		conversationTitleTestInput(t, s, fmt.Sprintf("compressed-h%d", i+1), conv.ID, input, i+1)
	}
	...
	if _, err := conversationTitleTestRun(t, s, conv.ID); err != nil {
		t.Fatal(err)
	}
	if meta := conversationTitleTestMeta(t, s, conv.ID); meta.Status != "done" || meta.OpeningTurns != 3 || meta.CallCount != 1 {
...
func TestOpeningRealModelDescription(t *testing.T) {
	...
	conversationTitleTestConversation(t, s, "real", "解释倒排索引", "default")
	conversationTitleTestInput(t, s, "real-h", "real", "解释倒排索引的基本工作原理", 1)
	if _, err := s.enqueue(context.Background(), "real", ""); err != nil {
		t.Fatal(err)
	}
	if _, err := conversationTitleTestRun(t, s, "real"); err != nil {
		t.Fatal(err)
	}
	meta := conversationTitleTestMeta(t, s, "real")
