...
ALTER TABLE public.local_workspaces
    ADD COLUMN read_policy VARCHAR(32) NOT NULL DEFAULT 'allow' CONSTRAINT chk_local_workspaces_read_policy CHECK (read_policy = 'allow'),
    ADD COLUMN write_policy VARCHAR(32) NOT NULL DEFAULT 'allow' CONSTRAINT chk_local_workspaces_write_policy CHECK (write_policy = 'allow');

-- +migrate Dialect sqlite
PRAGMA foreign_keys = OFF;
CREATE TABLE local_workspaces_with_policies (
    id TEXT PRIMARY KEY,
...
INSERT INTO local_workspaces_with_policies (
...
SELECT id, create_user_id, display_name, canonical_path, directory_identity,
       status, version, source, 'allow', 'allow', authorized_at,
       last_used_at, revoked_at, created_at, updated_at
FROM local_workspaces;
DROP TABLE local_workspaces;
ALTER TABLE local_workspaces_with_policies RENAME TO local_workspaces;
