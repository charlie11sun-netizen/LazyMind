...
ALTER TABLE public.local_workspaces
    ALTER COLUMN write_policy SET DEFAULT 'ask_before_write';
UPDATE public.local_workspaces SET write_policy = 'ask_before_write';
ALTER TABLE public.local_workspaces
    ADD CONSTRAINT chk_local_workspaces_write_policy CHECK (write_policy = 'ask_before_write');

-- +migrate Dialect sqlite
PRAGMA foreign_keys = OFF;
CREATE TABLE local_workspaces_write_policy_old (
    id TEXT PRIMARY KEY,
...
INSERT INTO local_workspaces_write_policy_old (
...
SELECT id, create_user_id, display_name, canonical_path, directory_identity,
       status, version, source, read_policy, 'ask_before_write', authorized_at,
       last_used_at, revoked_at, created_at, updated_at
FROM local_workspaces;
DROP TABLE local_workspaces;
ALTER TABLE local_workspaces_write_policy_old RENAME TO local_workspaces;
