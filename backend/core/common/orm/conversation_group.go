...
type ConversationGroup struct {
	...
	ProjectPath    *string    `gorm:"column:project_path;type:text;uniqueIndex:uk_conversation_projects_user_path,priority:3,where:kind = 'project' AND deleted_at IS NULL"`
	Pinned         bool       `gorm:"column:pinned;not null;default:false"`
	SortOrder      int64      `gorm:"column:sort_order;not null;default:0"`
	ID             string     `gorm:"column:id;type:varchar(36);primaryKey"`
	UserID         string     `gorm:"column:user_id;type:varchar(255);not null;uniqueIndex:uk_conversation_groups_user_name,priority:1,where:kind = 'group';uniqueIndex:uk_conversation_projects_user_path,priority:1,where:kind = 'project' AND deleted_at IS NULL"`
	Name           string     `gorm:"column:name;type:varchar(255);not null"`
	NormalizedName string     `gorm:"column:normalized_name;type:varchar(255);not null;uniqueIndex:uk_conversation_groups_user_name,priority:3,where:kind = 'group'"`
